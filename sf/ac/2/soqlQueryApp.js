// soqlQueryApp.js - Enhanced SOQL Editor with REST URL Support, REST/Tooling API, Improved Error Handling, and SOQL Query Autocomplete
    ...
    loadCommonQueries().then(() => {
        ...
        require(['vs/editor/editor.main'], function (monaco, $) {
            ...
            monaco.languages.registerCompletionItemProvider('sql', {
                triggerCharacters: [' ', ',', '\n', '.', 'S', 'F', 'A', 'C'], // Trigger on common SOQL keywords or letters
                provideCompletionItems: async function (model, position) {
                    ...
                    // Add common SOQL queries from JSON
                    ...
                    Object.entries(commonQueries).forEach(([queryName, query]) => {
                        if (queryName.toLowerCase().includes(prefix) || query.toLowerCase().includes(prefix)) {
                            // Format the query to add new lines before FROM and WHERE (if present)
                    ...
                    // Optionally, keep the existing SObject field suggestions (fetchSObjectFields)
