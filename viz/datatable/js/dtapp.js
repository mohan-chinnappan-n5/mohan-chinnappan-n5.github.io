...
const renderDataTable = (input) => {
    ...
    drawBtn.addEventListener('click', event => {
        drawChart(data, fieldTypes, xAxisSelect.value, yAxisSelect.value);
        getEle('vizBar').focus();
    ...
    for (const key in firstObject) {
        if (firstObject.hasOwnProperty(key)) {
            ...
            if (typeof value === "string") {
                ...
                fieldTypes[key] = data.every((item) => typeof item[key] === "string")
                    ? "nominal"
                    : "quantitative";
            } else if (typeof value === "number") {
                // Check if the field is quantitative (contains numeric values)
                fieldTypes[key] = data.every((item) => typeof item[key] === "number")
                    ? "quantitative"
                    : "nominal";
            } else {
