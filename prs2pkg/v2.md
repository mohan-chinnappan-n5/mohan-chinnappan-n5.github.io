# PRs to Package.xml Tool  - version 2
...
1. Save the script as `/usr/local/bin/prs-to-pkg-vscode-bash.sh`
2. Make it executable:  
   ```bash
   chmod +x /usr/local/bin/prs-to-pkg-vscode-bash.sh
   ```
...
2. Run the script with a **comma-separated list of PR numbers**:
   ```bash
   prs-to-pkg-vscode-bash.sh 1344,1356
   ```
